...
// PhoneFrame — iPhone 16 Pro Max device mockup, used on the marketing landing
// to frame app screenshots. Dependency-free, SSR-safe replacement for the
// @sneas/telephone web component (MIT, https://github.com/sneas/telephone): the
// bezel SVG below is vendored from that project (MIT-licensed). `mode` recolors
// the frame to contrast the page background (light page → dark frame, and vice
// versa), matching upstream behaviour. Styling lives in landing.css; the screen
// radius scales with the frame width via a container-query unit (cqw), so no
// ResizeObserver is needed.

import type { ReactNode } from "react";

const BEZEL_SVG = `<svg width="100%" viewBox="0 0 415 843" fill="none" xmlns="http://www.w3.org/2000/svg">
...
// Upstream recolors the frame from near-black to white for dark backgrounds.
const BEZEL_SVG_DARK = BEZEL_SVG.replace(/#0D0D0E/gi, "#ffffff");
...
export function PhoneFrame({
  ...
  return (
    <div className="phone-frame">
      <div className="phone-screenshot">{children}</div>
      <div
        className="phone-bezel"
        // Static vendored MIT SVG, no user input.
        // biome-ignore lint/security/noDangerouslySetInnerHtml: trusted static asset
        dangerouslySetInnerHTML={{ __html: mode === "dark" ? BEZEL_SVG_DARK : BEZEL_SVG }}
      />
    </div>
  );
}
